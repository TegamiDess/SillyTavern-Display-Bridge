...
Portrait adapter v11 (Display Bridge 0.21.0) adds optional `format.normalization.displayCleanup`: a unique list containing `setup-notes` and/or `roster-notes`. Discovery enables these only for the exact reviewed source deletion rules. Cleanup affects display copies, never saved messages or model requests; code examples and incomplete markers remain visible. Version 10 adds scene-field normalization and state-v3 request-context templates. Older profiles remain supported. See [automatic scene import](SCENE-AUTO-IMPORT.md) and `examples/scene-context-import-profile.json` at the repository root.

The early-version sections below describe the original handoff contract. Current replacement and lifecycle support is documented in [replacement/recovery](REPLACEMENT-RECOVERY.md) and [persistence](PERSISTENCE.md); current validation evidence is in the [release acceptance matrix](../RELEASE-ACCEPTANCE.md).

Portrait adapter v6 (Display Bridge 0.16.0) adds optional `format.details` to scene fragments: reviewed layers, period mappings, offset aliases, tooltips and bounded rich dialogue. Existing profiles retain their behavior. See [SCENE-ASSEMBLY.md](SCENE-ASSEMBLY.md) for the format and upgrade route.
...
- A standalone profile can be loaded and exported in Display Bridge settings. Loading stages it for review; applying replaces panel selections for that character. A card author can place the same object under `data.extensions.display_bridge_profile` in a V3 `card.json`. The configured CHARX exporter can package the applied profile with local images; see [EXPORT.md](../EXPORT.md). It creates a new archive without changing the original.
...
- This version does not import arbitrary matchers/actions/bindings, run Lua, automatically execute source story-variable effects, insert missing roster markers into messages, or ensure all Risu features are translated. Explicit v8 profiles can configure reviewed typed scene updates and model context; see SCENE-STATE.md. Adapter version 1 references the reviewed presentation definitions described in `ACTIONS.md`.
...
Portrait adapter version **7** adds optional `source.sceneControls` for snapshot-based rosters and declared local tracks. It requires `scene-fragments`; older adapter versions reject this field. See [configuration and snapshot schema](SCENE-CONTROLS.md).

## Scene state in 0.18.0

Portrait adapter version **8** adds validated `source.sceneState` for explicit typed updates, startup choices, saved chat ancestry and declared model context. Versions 1–7 remain readable. The mapping editor preserves the configuration; edit it through Advanced preset JSON or profile import. Profile/card exports include defaults and choices, never active chat journals. See [SCENE-STATE.md](SCENE-STATE.md) and `examples/scene-state-profile.json`.

## Automatic state profiles in 0.19.0

Portrait adapter v9 / sceneState v2 adds bounded literal assignments, finite value lookups, case policy and integer value constraints for recognized source imports. v8/state-v1 remains supported. See [automatic import](SCENE-AUTO-IMPORT.md) and `examples/scene-auto-import-profile.json`.
